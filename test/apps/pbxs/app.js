define(function(require){
	...
	var app = {
		...
		listAvailablePbxs: function() {
			return ['agfeo', 'alcatel', 'allworks', 'altigen', 'asterisk', 'avaya', 'bluebox', 'cisco', 'digium', 'epygi', 'freepbx', 'freeswitch', 'mitel', 'objectworld', 'other', 'panasonic', 'pingtel', 'responsepoint', 'samsung', 'shoretel', 'siemens', 'sutus', 'talkswitch', 'threecom', 'taridium'];
		},
		...
		renderEndpoint: function(data, endpointData, target, callbacks, parent) {
			...
			$('.pbx-brand-list .pbx', endpointHtml).each(function() {
				if($(this).data('pbx_name') === endpointData.server_type) {
					...
					$('.pbx-brand-list .pbx:not(.selected)', endpointHtml).css('opacity', '0.5');
					return false;
				}
			});
			...
			$('.pbx-brand-list .pbx', endpointHtml).click(function() {
				$('.pbx-brand-list .pbx', endpointHtml).removeClass('selected').css('opacity', '0.5');
				$(this).addClass('selected');
		...
		renderPbxsManager: function(data, endpointData, target, callbacks) {
			...
			pbxsManager.on('click', '.regextern-number', function() {
				var regexternCell = $(this).parents('.number-wrapper').first(),
					phoneNumber = regexternCell.data('phone_number');
				if(phoneNumber) {
					var args = {
						phoneNumber: phoneNumber,
						callbacks: {

						...
						self.userList(function(users) {
							...
							if($('#user_selector option:selected', popup_html).val() == undefined) {
								$('#edit_link', popup_html).hide();
                                                        ...
                                                        popup_html.find('#user_timezone').chosen({search_contains: true, width: "220px"});
                                                        popup_html.find('#user_language').chosen({search_contains: true, width: "220px"});
		...
		userRender: function(data, target, callbacks) {
			...
			monster.ui.validate(user_form, {
				...
				messages: {
					...
					'caller_id.emergency.number': { regex: self.i18n.active().callflows.user.validation.caller_id.number }
                        ...
                        timezone.populateDropdown(user_html.find('#user_timezone'), data.data.timezone||'inherit', {inherit: self.i18n.active().defaultTimezone});
                        language.populateDropdown(user_html.find('#user_language'), data.data.language||'inherit', {inherit: self.i18n.active().defaultLanguage});
			...
			$('.user-save', user_html).click(function(ev) {
				...
				if (monster.ui.valid(user_form)) {
					...
					if(! self.userCleanFormData(form_data))
						toastr.error(self.i18n.active().callflows.timeofday.there_were_errors_on_the_form, '', {"timeOut": 10000});
					...
					toastr.error(self.i18n.active().callflows.user.there_were_errors_on_the_form, '', {"timeOut": 10000});
				}
			});

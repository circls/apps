define(function(require){
	...
	var app = {
		...
		i18n: { 
                "en-US": { customCss: true },
                "es-ES": { customCss: true },
                "de-DE": { customCss: true },
                "dk-DK": { customCss: true },
                "fr-FR": { customCss: true },
                "it-IT": { customCss: true },
                "nl-NL": { customCss: true },
                "ro-RO": { customCss: true },
                "pt-PT": { customCss: true },
                "zh-CN": { customCss: true },
                "ru-RU": { customCss: true }
		},
		...
		bindAccountSettingsEvents: function(template, data) {
			...
			template.find('.upload-input').fileUpload({
				...
				error: function(errors) {
					if(errors.hasOwnProperty('size') && errors.size.length > 0) {
						toastr.error(self.i18n.active().callflows.accountSettings.musicOnHold.fileTooBigAlert, '', {"timeOut": 10000});
					}
					template.find('.upload-div input').val('');
					mediaToUpload = undefined;
				}
			...
			template.find('.upload-submit').on('click', function() {
				if(mediaToUpload) {
					...
					toastr.error(self.i18n.active().callflows.accountSettings.musicOnHold.emptyUploadAlert, '', {"timeOut": 10000});
				}
			});
		...
		renderButtons: function() {
			...
			$('.save', buttons).click(function() {
				if(self.flow.numbers && self.flow.numbers.length > 0) {
					...
					toastr.error(self.i18n.active().callflows.invalid_number + '<br/><br/>' + self.i18n.active().oldCallflows.please_select_valid_number, '', {"timeOut": 10000});
				}
			});
			...
			$('.delete', buttons).click(function() {
				if(self.flow.id) {
					...
					toastr.error(self.i18n.active().callflows.this_callflow_has_not_been_created, '', {"timeOut": 10000});
				}
			});
		...
		getUIFlow: function() {
			...
			$('.node', layout).each(function() {
				...
				if (node.actionName == 'root') {
					...
					node_html = $(monster.template(self, 'root', { name: self.flow.name || self.i18n.active().oldCallflows.callflow }));
						...
						self.listNumbers(function(phoneNumbers) {
							...
							$('.add_number', popup).click(function(event) {
								...
								if(number !== 'select_none' && number !== '') {
									...
									toastr.error(self.i18n.active().oldCallflows.you_didnt_select, '', {"timeOut": 10000});
								}
							});
						});
					});
		...
		save: function() {
			...
			if(self.flow.numbers && self.flow.numbers.length > 0) {
				...
				toastr.error(self.i18n.active().oldCallflows.you_need_to_select_a_number, '', {"timeOut": 10000});
			}
		},

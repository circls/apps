define(function(require){
	...
	var app = {
		...
		subscribe: {
			'voip.devices.userrender': 'devicesRender',
			'voip.devices.userrenderAdd': 'devicesRenderAdd',
			'voip.devices.userrenderChange': 'modelsRenderChange',
			'voip.devices.usereditDevice': 'devicesRenderEdit'
		},
...
//						self.devicesRender();
		...
		templateRenderChange: function(args) {
			...
			if(args.data.device_type === 'sip_device' && monster.config.api.provisioner) {
				self.devicesGetDevice(args.deviceId, function(dataDevice) {
					if(typeof dataDevice.provision !== "undefined") {
						template = 'true';
						if(typeof dataDevice.provision.feature_keys == "object") {
						...
						} else args.data.provision.feature_keys = {};
						if(args.renderedit !== 'true') {
...
//				dialogClass: 'overflow-visible'
		...
		devicesFormatData: function(data, dataList) {
			...
			_.each(data.listClassifiers, function(restriction, name) {
				...
				if(typeof data.accountLimits !== 'undefined') {

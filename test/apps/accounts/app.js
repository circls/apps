	...
	var app = {
									...
									self.callApi({
										...
										success: function(data, status) {
											var newLimits = {
												...
												allow_postpay: formData.limits.allow_postpay,
												inbound_trunks: parseInt(formData.limits.inbound_trunks, 0),
												outbound_trunks: parseInt(formData.limits.outbound_trunks, 0),
												twoway_trunks: parseInt(formData.limits.twoway_trunks, 0),
												calls: parseInt(formData.limits.calls, 2),
												burst_trunks: parseInt(formData.limits.burst_trunks, 0),
												resource_consuming_calls: parseInt(formData.limits.resource_consuming_calls, 2)
											};
											self.callApi({
		...
		updateUIRestrictionsFromServicePlan: function(template, accountData, servicePlan, callback) {
				...
				newUIRestrictions = {
					ui_restrictions: {
						myaccount: {
							...
							burst_trunks: {
								show_tab: getRestrictionValue('burst_trunks')
							},
							calls: {
								show_tab: getRestrictionValue('calls')
							},
							resource_consuming_calls: {
								show_tab: getRestrictionValue('resource_consuming_calls')
							}
						}
					}
				},
			...
			self.updateData(accountData, newUIRestrictions, function(data) {
				...
				setCheckboxValue('calls', data);
				setCheckboxValue('burst_trunks', data);
				setCheckboxValue('resource_consuming_calls', data);
		...
		// If a service plan has a rate for one of the Allotments, we should display the my account category,
		// so this function checks that and returns a map of Allotments type with a boolean telling whether it should be shown or not
		getUIRestrictionForAllotments: function(Allotments) {
			var self = this,
				hasAllotments = servicePlan.hasOwnProperty('data') && servicePlan.data.hasOwnProperty('plan') && servicePlan.data.plan.hasOwnProperty('Allotments'),
				Allotments = hasAllotments ? servicePlan.data.plan.Allotments : {},
				formattedData = {},
				getUIRestrictionValue = function(key) {
					var value = false;

					if(hasAllotments && Allotments.hasOwnProperty(key) && Allotments[key].hasOwnProperty('rate')) {
						value = true;
					}
			...
			if(hasAllotments) {
				_.each(Allotments, function(v, k) {
					formattedData[k] = getUIRestrictionValue(k);
				});
			}
		...
		// Function used when user updates service plan via update of the account page.
		// If he selects a new service plan we need to automatically turn on/off the Allotments settings based on the service plan
		updateRestrictionFromAllotments: function(template, accountData, servicePlan, callback) {
			var self = this,
				servicePlanRestrictions = self.getUIRestrictionForServicePlan(servicePlan),
				getRestrictionValue = function(key) {
					return servicePlanRestrictions.hasOwnProperty(key) ? servicePlanRestrictions[key] : false;
			...
			self.updateData(accountData, newUIRestrictions, function(data) {
                ...
                renderAllotmentEdit: function(parent, allotmentId) {
                        var self = this,
                                getallotmentData = function(allotmentId, callback) {
                                        monster.parallel({
                                                getallotment: function(parallelCallback) {
                                                        self.getAvailableallotments(function(data) {
                        ...
                        getallotmentData(allotmentId, function(allotmentData) {0
                                var allotmentListI18n = self.i18n.active().allotments.allotmentList;
                                        allotmentList = monster.util.sort(_.map(allotmentData.allotmentList, function(val) {
                                                return {
                                                        id: val.id,
                                                        name: val.id in allotmentListI18n ? allotmentListI18n[val.id].name : val.name,
                                                        description: val.id in allotmentListI18n ? allotmentListI18n[val.id].description : val.description
                                                }
                                        })).concat({
                                                id: 'all',
                                                        name: allotmentListI18n.all.name,
                                                        description: allotmentListI18n.all.description
                                        }),
                                        template = $(monster.template(self, 'allotments', {
                                                allotmentList: allotmentList,
                                                allotment: allotmentData.allotmentDetails,
                                                groups: (Object.keys(self.uiFlags.account.get('groups') || {})).sort()
                                        }));
                                // Iterate through custom_data to print current custom_data
                                if(allotmentData.allotmentDetails.custom_data) {
                                        _.each(allotmentData.allotmentDetails.custom_data, function(val, key) {
                                                var customDataTemplate = monster.template(self, 'allotments-customDataRow', {
                                                        key: key,
                                                        value: val
                                                });
                                ...
                                monster.ui.validate(template.find('#allotment_edition_form'), {
                                        rules: {
                                                'uri': {
                                                        url: true
                                                }
                                ...
                                self.bindallotmentEditEvents(template, allotmentData.allotmentDetails);
                                parent
                                        .find('.allotments-container')
                                        .empty()
                                        .append(template);
                        });
                },
		...
		editAccount: function(params) {
			...
			if(monster.apps.auth.isReseller) {
				var $btn_change = contentHtml.find('#accountsmanager_serviceplan_change'),
					...
					$allotadd = contentHtml.find('#accountsmanager_allotments_add');
					$allotedit = contentHtml.find('#accountsmanager_allotments_edit');

                                        $allotadd.on('click', function() {
                                                self.renderAllotmentEdit(template, null);
                                        });

                                        $allotedit.on('click', function(e){
                                                var allotmentId = $(this).data('id');
                                                self.renderAllotmentEdit(template, allotmentId);
                                        });
			...
			contentHtml.find('#accountsmanager_numbersfeatures_save').on('click', function() {
				self.callApi({
								...
								monster.pub('common.accountBrowser.getBreadcrumbsList', {
									...
									callback: function(breadcrumbs) {
										self.render({
											...
											callback: function() {
												toastr.success(self.i18n.active().toastrMessages.numbersfeaturesUpdateSuccess, '', {"timeOut": 5000});
											}
										});
									}
								});
					...
					error: function(data, status) {
						toastr.error(self.i18n.active().toastrMessages.numbersfeaturesUpdateError, '', {"timeOut": 5000});
					}
				});
			});
		...
		*/
		renderLimitsTab: function(params) {
			var self = this,
				...
				callsTrunksDiv = tabContentTemplate.find('.trunks-div.calls');
				burstTrunksDiv = tabContentTemplate.find('.trunks-div.burst_trunks');
				resourceconsumingCallsDiv = tabContentTemplate.find('.trunks-div.resource_consuming_calls');
			...
			parent.find('#accountsmanager_limits_save').click(function(e) {
				...
				var newTwowayValue = twowayTrunksDiv.find('.slider-div').slider('value'),
					...
					newCallsValue = callsTrunksDiv.find('.slider-div').slider('value'),
					newBursttrunksValue = burstTrunksDiv.find('.slider-div').slider('value'),
					newResourceconsumingcallsValue = resourceconsumingCallsDiv.find('.slider-div').slider('value'),
					...
					allowPostpay = tabContentTemplate.find('.allow-postpay-ckb').is(':checked');
							...
							self.callApi({
								...
								data: {
									...
									data: $.extend(true, {}, limits, {
										...
										allow_postpay: allowPostpay,
										call_restriction: callRestrictions,
										calls: newCallsValue,
										burst_trunks: newBursttrunksValue,
										resource_consuming_calls: newResourceconsumingcallsValue,
									})
								},
								success: function(data, status) {
		...
		getLimitsTabContent: function(params) {
			var self = this,
				...
				template = $(monster.template(self, 'limitsTabContent', {
					...
					allowPostpay: limits.hasOwnProperty('allow_postpay') ? limits.allow_postpay : false,
				...
				calls = limits.calls || 0,
				callsTrunksDiv = template.find('.trunks-div.calls'),
				burst_trunks = limits.burst_trunks || 0,
				burstTrunksDiv = template.find('.trunks-div.burst_trunks'),
				resource_consuming_calls = limits.resource_consuming_calls || 0,
				resourceconsumingCallsDiv = template.find('.trunks-div.resource_consuming_calls'),
			...
			createSlider({
				trunksDiv: burstTrunksDiv,
				minValue: 0,
				maxValue: 100,
				currentValue: burst_trunks
			});

			createSlider({
				trunksDiv: resourceconsumingCallsDiv,
				minValue: 0,
				maxValue: 100,
				currentValue: resource_consuming_calls
			});
			...
			callsTrunksDiv.find('.slider-value').html(calls);
			burstTrunksDiv.find('.slider-value').html(burst_trunks);
			resourceconsumingCallsDiv.find('.slider-value').html(resource_consuming_calls);
		...
		*/
		...
		getBalance: function(accountId, success, error) {
			...
			self.callApi({
                ...
                getAllotments: function(callback){
                        var self = this;

                        self.callApi({
                                resource: 'allotments.list',
                                data: {
                                        accountId: self.accountId
                                },
                ...
                updateAllotment: function(allotmentId, data, callback){
                        var self = this;

                        self.callApi({
                                resource: 'allotments.update',
                                data: {
                                        accountId: self.accountId,
                ...
                deleteAllotment: function(allotmentId, callback){
                        var self = this;
                        self.callApi({
                                resource: 'allotments.delete',
                                data: {
                                        accountId: self.accountId,
                                        allotmentId: allotmentId,
                                        data: {}

...
if(DEBUG_FUNCTION == 'd' || DEBUG_FUNCTION == 'yealink' && ($_REQUEST['user_agent'] && $_REQUEST['mac'] && $_REQUEST['qpass'] && $_REQUEST['firmware']))
{
    $header=Yealink_decode_HTTP_header();
    global $debug; $debug['yealink/dheader'][] = "(yealink 1) ".$header[0]."-".$header[1]."-".$header[2]."-".$header[3]."-".$header[4];
    $nonepass = false;
    $model = strtolower($header[0].$header[1]);
    $mac = strtoupper(str_replace(":","",$header[3]));
    $firmware = $header[2];
    if($mac == strtoupper(str_replace(":","",$_REQUEST['mac'])) && urldecode($_REQUEST['firmware']) == $firmware)
    ...
    if($_REQUEST['pass']) $qpass = urldecode($_REQUEST['pass']);
    global $debug; $debug[] = array(level=>'d',status=>'info',file=>__FILE__.":".__LINE__,log=>'('. __FUNCTION__ .') '."rem_ip=$remote_ip, pass=$qpass, mac=$mac");
...
function Yealink_decode_HTTP_header()
{
 if($_REQUEST['user_agent'] != false)
    $user_agent = urldecode($_REQUEST['user_agent']);
 else
...
function get_yealink_cfg($mac)
{
    ...
    $durl = urldecode(parse_url($_SERVER['REQUEST_URI']));
